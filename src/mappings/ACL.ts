...
export function handleSetPermission(event: SetPermissionEvent): void {
  ...
  const granteeAddres = event.params.entity
  ...
  const permissionId = appAddress
    ...
    .concat(roleHash.toHexString())
    .concat('-')
    .concat(granteeAddres.toHexString())
  ...
  if (allowed) {
    ...
    if (permission == null) {
      ...
      permission.granteeAddres = event.params.entity
    }
...
export function handleSetPermissionParams(
  ...
  const paramsLength = acl
    .getPermissionParamsLength(granteeAddres, appAddress, roleHash)
    .toI32()
